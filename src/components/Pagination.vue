    ...
    <div :class="class" class="mx-auto px-4 py-6 sm:px-6 lg:px-8">
      ...
      <!-- PAGINATION -->
        ...
        <div class="flex flex-1 justify-between sm:hidden">
            <button @click="prevPage()" :disabled="paginate.page == 1" class="relative inline-flex items-center rounded-md border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-gray-200 hover:bg-white/10 disabled:opacity-75">{{t('paginator.previous')}}</button>
            <button  @click="nextPage()" v-if="paginate.totalPages > paginate.page" class="relative ml-3 inline-flex items-center rounded-md border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-gray-200 hover:bg-white/10">{{ t('paginator.next') }}</button>
        </div>
